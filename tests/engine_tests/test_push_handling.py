...
# ---------------------------------------------------------------------------
# 3. Calibration ingest: pushes are zero-information
# ---------------------------------------------------------------------------

class CalibrationPushIngestTests(unittest.TestCase):
    """`_ingest_one` is the per-leg ingestion entry point used by both the
    isotonic curve fitter and the heatmap aggregator. Pushes must NOT
    contribute outcome counts."""

    def test_ingest_one_push_does_not_record_outcome(self):
        from engine import isotonic_calibration as iso

        # Build a fresh, well-formed state via the module's own factory so
        # the `global` / `leagues` / `props` shape matches whatever the
        # real curve fitter expects.
        state = iso._empty_state()
        now = datetime.now(timezone.utc)
        recent = now - timedelta(days=1)

        # Sanity: a "hit" ingest writes outcome counts under the (NBA,
        # Points, over) prop bucket.
        iso._ingest_resolved_row(state, "NBA", "Points", "over",
                                 true_prob=0.60, result="hit",
                                 resolved_at=recent, closing_prob=None, now=now)
        hit_props = dict(state.get("props") or {})
        self.assertIn("NBA|Points|over", hit_props,
                      "hit ingest must create the (league|prop|side) accumulator")

        # A push for a *different* prop must not mint a new accumulator —
        # it carries no outcome signal and no CLV (closing_prob=None).
        iso._ingest_resolved_row(state, "NBA", "Rebounds", "over",
                                 true_prob=0.55, result="push",
                                 resolved_at=recent, closing_prob=None, now=now)
        post_props = dict(state.get("props") or {})
        new_keys = set(post_props.keys()) - set(hit_props.keys())
        self.assertFalse(
            new_keys,
            f"push (no closing_prob) must not create new prop buckets, got {new_keys}",
        )

        # And the existing Points bucket must be byte-for-byte unchanged
        # by a push to a *different* prop.
        self.assertEqual(post_props["NBA|Points|over"],
                         hit_props["NBA|Points|over"],
                         "push must not retroactively mutate other buckets")

    def _assert_no_outcome_signal(self, v):
        """Recursively walk an ingest-state value; fail if any leaf number
        looks like an outcome accumulation. Pushes may legitimately bump
        zero-weighted CLV-only accumulators if `closing_prob` is given,
        but our test passes closing_prob=None so the entire push call
        should be a no-op."""
        if isinstance(v, dict):
            for sub in v.values():
                self._assert_no_outcome_signal(sub)
        elif isinstance(v, (list, tuple)):
            for sub in v:
                self._assert_no_outcome_signal(sub)
        elif isinstance(v, (int, float)):
            self.assertEqual(v, 0.0,
                             "push (no closing_prob) must not add weight to ingest state")


# ---------------------------------------------------------------------------
# 4. Database resolved-rows IN-list omits push
# ---------------------------------------------------------------------------

class CalibrationQueryFilterTests(unittest.TestCase):
    """The resolved-rows query in `engine/calibration.py::_load_legs` is the
    feeder for Brier/log-loss; pushes must not appear in the IN-list (or
    they'd be treated as misses by the outcome bit derivation)."""

    def test_push_not_in_resolved_in_list(self):
        from engine import calibration
        # The _load_legs function references the IN list inline; we read
        # the source directly because the query builder is too
        # database-coupled to introspect.
        import inspect
        src = inspect.getsource(calibration._load_resolved_rows)
        # Push must not be a member of the .in_("result", [...]) call.
        self.assertNotIn('"push"', src)
        self.assertNotIn("'push'", src)

    def test_resolved_constant_excludes_push(self):
        from engine.calibration import evaluate_analytics  # noqa: F401
        # _RESOLVED is module-private; pull it via the source.
        from engine import calibration
        # Defensive: walk module dict for the literal we know is there.
        # If the constant is renamed, this test stays trivially green; the
        # stronger assertion lives inside _load_legs (above) where the
        # filter is actually applied.
        for name in dir(calibration):
            if name.startswith("_RESOLVED") or name.endswith("_RESOLVED"):
                resolved = getattr(calibration, name)
                if isinstance(resolved, (set, list, tuple, frozenset)):
                    self.assertNotIn("push", resolved,
                                     f"{name} must not include push")
